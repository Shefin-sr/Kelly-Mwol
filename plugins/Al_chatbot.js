...
if (conf.LANG == 'TR') {
    ...
    already_on = 'KellyMwol yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'KellyMwol yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'KellyMwol, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'KellyMwol, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Turns on AI powered chatbot on to your account!'
    already_on = 'KellyMwol chatbot is already on.'
    already_off = 'KellyMwol chatbot is currently turned off!.'
    succ_on = 'KellyMwol chatbot on! Restarting to make chatbot ✅'
    succ_off = 'KellyMwol chatbot off Restarting to make normal  ❤️'
}
if (conf.LANG == 'ML') {
    ...
    succ_on = 'KellyMwol പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'KellyMwol സെമി-ഫങ്ഷണൽ ആയി സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

New.addCommand({ pattern: 'kellymwol ?(.*)', desc: _dsc, fromMe: true,dontAddCommandList: true, usage: '.kellymwol on / off' }, (async (message, match) => {
